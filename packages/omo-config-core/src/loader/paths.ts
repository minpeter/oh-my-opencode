...
export function resolveOmoConfigNamespace(env: OmoConfigEnv = process.env): string {
  const namespace = env.OMO_CONFIG_NAMESPACE ?? ".omo"
  if (!/^\.[a-z][a-z0-9_-]*$/i.test(namespace)) {
    throw new TypeError("OMO_CONFIG_NAMESPACE must be a single dot-directory name")
  }
  return namespace
}
...
export function resolveUserOmoConfigDirectory(env: OmoConfigEnv = process.env): string {
  ...
  const directory = env.OMO_USER_CONFIG_DIR
  if (directory !== undefined) {
    if (!isAbsolute(directory)) throw new TypeError("OMO_USER_CONFIG_DIR must be an absolute directory")
    return resolve(directory)
  }
  return join(resolveHomeDir(env), namespace)
}
...
function detectOmoJsonPath(dir: string, fileSystem: OmoConfigReadFileSystem, namespace: string): string | null {
  const omoDir = join(dir, namespace)
...
export function findProjectConfigPathsFarthestFirst(
  ...
  for (let depth = 0; depth < MAX_PROJECT_CONFIG_DIRECTORY_DEPTH; depth += 1) {
    ...
    const configPath = isHomeDir ? null : detectOmoJsonPath(currentDir, fileSystem, namespace)
...
export function resolveOmoConfigPaths(options: ResolveOmoConfigPathsOptions): readonly OmoConfigPathCandidate[] {
  ...
  const projectPaths = env.OMO_DISABLE_PROJECT_CONFIG === "1" ? [] : findProjectConfigPathsFarthestFirst(
    options.cwd,
    resolveHomeDir(env),
    fileSystem,
    ACCOUNT_HOME_DIR,
    resolveOmoConfigNamespace(env),
  )
  return [
    { path: userPath, scope: "user" },
    ...projectPaths.filter((path) => path !== userPath)
      .map((path): OmoConfigPathCandidate => ({ path, scope: "project" })),
  ]
}
