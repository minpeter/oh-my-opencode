...
| `dag-runtime.ts` | DAG composition root: one shared manager/store graph feeding the tool, scheduler, RPC bridge, TUI, wake emitter, and lifecycle adapters. `attach` runs recovery and, for every own run skipped as `live_lease`, arms a lease watch (`deps.leaseWatch`: probe + timers, injectable) that re-runs recovery once the predecessor host pid exits; `detach`/`pauseForShutdown`/`dispose` cancel the watches. |
