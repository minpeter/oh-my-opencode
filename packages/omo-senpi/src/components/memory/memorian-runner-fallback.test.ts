...
import { TOOL_CALL_JUDGE_DEADLINE_MS } from "./memorian-trigger"
import { DEAD_PRIMARY, FALLBACK_PROVIDER, fallbackProviderHarness, HEALTHY_FALLBACK } from "./memorian-runner.fallback-support"
...
describe("MemorianGateRunner live fallback rotation", () => {
  ...
  test("#given every rung returns 503 #when the judge runs on the tool_call deadline #then the engine settles the turn and the gate reports child_failed_upstream long before the deadline", async () => {
    ...
    expect(harness.calls).toEqual([DEAD_PRIMARY, DEAD_PRIMARY, HEALTHY_FALLBACK])
    expect(result).toMatchObject({ status: "failed", cause: "child_failed_upstream", reason: expect.stringContaining("503") })
    expect(elapsedMs).toBeLessThan(SETTLE_BUDGET_MS)
  })
})
