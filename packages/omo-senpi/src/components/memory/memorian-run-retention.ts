...
export type MemorianRunStatus = "completed" | "failed" | "dropped"
export type MemorianRunWarn = (message: string, fields?: Readonly<Record<string, unknown>>) => void
export type RemoveMemorianRun = (path: string) => Promise<void>

export interface WriteMemorianRunOutcomeInput {
  ...
  readonly now: () => Date
  readonly warn?: MemorianRunWarn
}

export interface PruneMemorianRunsInput {
  readonly recallDir: string
  readonly now: () => Date
  readonly maxAgeMs?: number
  readonly maxNudgedAgeMs?: number
  readonly remove?: RemoveMemorianRun
  readonly warn?: MemorianRunWarn
}
...
export interface CreateThrottledPruneInput {
  ...
  readonly prune?: (input: PruneMemorianRunsInput) => Promise<{ readonly removed: number; readonly kept: number }>
  readonly warn?: MemorianRunWarn
}

export async function writeMemorianRunOutcome(options: WriteMemorianRunOutcomeInput): Promise<void> {
  try {
    const payload = {
      version: 1,
      runId: options.runId,
    ...
    options.warn?.("memorian run outcome write failed", { runDir: options.runDir, error: describe(error) })
  }
}

export async function pruneMemorianRuns(
  options: PruneMemorianRunsInput,
  ...
  const names = await readdir(runsDir).catch((error: unknown) => {
    ...
    options.warn?.("memorian run prune listing failed", { path: runsDir, error: describe(error) })
    return []
  })
  let removed = 0
  let kept = 0
  ...
  for (const name of tombstones) {
    if (await removePath(join(runsDir, name), remove, options.warn, "memorian run tombstone cleanup failed")) {
      removed += 1
    }
  }
...
export function createThrottledPrune(options: CreateThrottledPruneInput): () => void {
  ...
  const prune = options.prune ?? pruneMemorianRuns
  ...
  return () => {
    ...
    void (async () => {
      try {
        ...
        options.warn?.("memorian run prune failed", { error: describe(error) })
      } finally {
        inFlight = false
      }
    })()
...
async function claimAndRemove(
  runsDir: string,
  name: string,
  remove: RemoveMemorianRun,
  warn: MemorianRunWarn | undefined,
): Promise<boolean> {
  ...
  try {
    ...
    warn?.("memorian run prune rename failed", { path: from, error: describe(error) })
    return false
  }
  return removePath(tombstone, remove, warn, "memorian run prune remove failed")
}
