...
import {
  ...
  pruneMemorianRuns,
  writeMemorianRunOutcome,
} from "./memorian-run-retention"
...
async function tmp(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "omo-memorian-retention-"))
  roots.push(root)
  return root
}
...
describe("writeMemorianRunOutcome", () => {
  test("#given a completed status with nudged paths #when writeMemorianRunOutcome runs #then outcome.json records them", async () => {
...
describe("pruneMemorianRuns", () => {
  ...
  test("#given a 29-day-old nudged dir #when pruning at 29 then 31 days #then it is kept then removed", async () => {
    ...
    expect(await pruneMemorianRuns({ recallDir, now: () => new Date(NOW.getTime() + 2 * DAY) })).toEqual({
      removed: 1,
      kept: 0,
    })
  ...
  test("#given a stale .prune-x tombstone #when pruning #then the tombstone is removed", async () => {
    ...
    await pruneMemorianRuns({ recallDir, now: () => NOW })
  ...
  test("#given a remove seam that throws ENOENT #when pruning #then it does not throw", async () => {
    ...
    await expect(pruneMemorianRuns({
      recallDir,
      now: () => NOW,
